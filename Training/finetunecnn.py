...
def main():
    ...
    params_rnn = list(decoder.parameters())
    params_cnn=list(list(encoder.resnet.parameters()))
    optimizer_rnn = torch.optim.Adam(params_rnn, lr=config.learning_rate)
    optimizer_cnn = torch.optim.Adam(params_cnn, 1e-5)
		...
		for i, (images, captions, lengths,img_ids) in enumerate(train_loader):
			...
			torch.save(encoder.state_dict(), 
				   os.path.join(config.teacher_cnn_path, 
						'encoder-%d-%d_finetune.pkl' %(epoch+1, i+1)))
			torch.save(decoder.state_dict(), 
				   os.path.join(config.teacher_lstm_path, 
						'decoder-%d-%d_finetune.pkl' %(epoch+1, i+1)))
		'''
